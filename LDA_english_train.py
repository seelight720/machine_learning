...
class GLDA(object):
    ...
    def lda_train(self, num_topics, datafolder, middatafolder, dictionary_path=None, corpus_path=None, iterations=5000,
                  passes=1, workers=3):
        time1 = datetime.now()
        num_docs = 0
        doclist = []
        ...
        print(
            'lda training time cost is : %s, all time cost is : %s ' % (datetime.now() - time2, datetime.now() - time1))
        # 模型的保存/ 加载
