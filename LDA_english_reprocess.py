...
class NlpPreProcess(object):
    ...
    def generate_dict_from_file2(self, filename):
        """读取json文件，返回字典数据"""
        dic={}
        with codecs.open(filename, 'r', 'utf-8-sig') as source_file:
            for line in source_file:
                try:
                    arrayline=line.split('|')
                    # dic = json.loads(line.strip())
                    dic[arrayline[0]]=arrayline[1]
                except:
                    pass
        return dic
...
if __name__ == '__main__':
    ...
    nlp_preprocess.preprocess_file('answers.csv', source_folder, dest_folder)
